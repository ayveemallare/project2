...
class TemplateManager extends PKPTemplateManager {
	...
	function TemplateManager($request = null) {
		...
		if (!defined('SESSION_DISABLE_INIT')) {
			...
			if (isset($journal)) {
				...
				/* Checks if user's role is "author"
	 			 * Added Aug 3, 2011, MSB
				 * */
				$this->assign('isAuthor', $this->isAuthor($journal->getId())); 
			echo	$this->assign('isEditor', $this->isEditor($journal->getId())); 
	...
	function isAuthor($journalId) {
		return Validation::isAuthor($journalId); 
	}

	/** Checks if user is editor/sectioneditor
	 *  Added Aug 3, 2011, MSB
	 * */

	function isEditor($journalId) {
		return (Validation::isEditor($journalId) || Validation::isSectionEditor($journalId)); 
	}
