...
class UserHandler extends Handler {
	...
	function index() {
		...
		if ($journal == null) { // Curently at site level
			...
			$this->getRoleDataForJournal($userId, $journalId, $submissionsCount, $isValid, $isAuthor);
			
			// Added Aug3, 2011, MSB
			if($isAuthor){
				$templateMgr->assign('pageCrumbTitle', 'author.submissions.mySubmissions');
			}
	 ...
	 */
	function getRoleDataForJournal($userId, $journalId, &$submissionsCount, &$isValid, &$isAuthor) {
		...
		if (Validation::isAuthor($journalId)) {
			...
			$isAuthor = true; //Added Aug3, 2011, MSB
		}
